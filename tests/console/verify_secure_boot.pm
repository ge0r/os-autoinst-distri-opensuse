...
sub run {
    ...
    # The final octal digit shows whether Secure Boot is enabled (1) or disabled (0)
    my $secure_boot_octal = substr($octal_str, -1);
    die 'Unexpected values in Secure Boot file' unless (length($octal_str) == 17) and ($secure_boot_octal =~ /0|1/); 
    my $secure_boot = $secure_boot_octal ? 'enabled' : 'disabled';
